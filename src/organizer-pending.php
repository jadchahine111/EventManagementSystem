...
// Delete events published by the organizer that are not verified yet and have ended
$sqlDelete = "DELETE FROM events WHERE organizer_email = '$email' AND is_verified = 0 AND event_date_finish <= NOW()";
$deleteResult = mysqli_query($con2, $sqlDelete);

if (!$deleteResult) {
    die("Delete query failed: " . mysqli_error($con2));
}
...
<?php
  if (isset($_GET['id'])) {
    ...
    if (isset($_GET['action']) && $_GET['action'] === 'reject4') {
                    ...
                    <tr>
                        ...
                        <td class="actions-cell">
                            <div class="buttons right nowrap">
                                <a href="views-event-org.php?id=<?php echo $row['id']; ?>&source=organizer-pending.php" class="button small blue --jb-modal" data-target="sample-modal-2">
                                    <span class="icon"><i class="mdi mdi-eye"></i></span>
                                </a>
                                <a href="organizer-pending.php?id=<?php echo $row['id']; ?>&action=reject4" class="button small red">
