...
if (isset($_POST["add_user"])) {
    ...
    $first_name = isset($_POST["first_name"]) ? ucfirst(strtolower(trim($_POST["first_name"]))) : '';
    $last_name = isset($_POST["last_name"]) ? ucfirst(strtolower(trim($_POST["last_name"]))) : '';  
    $email1 = htmlspecialchars($_POST["email1"]);
    $file_no = htmlspecialchars($_POST["file_no"]);
    ...
    if (empty($first_name) || empty($last_name) || empty($email1) || empty($phone_number) || empty($_POST["password"]) || empty($_POST["retype_password"])) {
...
if ($emailCheckResult !== false && $emailCheckResult->num_rows > 0) {
    $errors[] = "Email already used!";
} else {
    // If there are no errors, proceed with inserting into the database
    if (empty($errors)) {
        // SQL query to insert user data into the 'users' table
        $sql = "INSERT INTO users (first_name, last_name, email, file_no, phone_number, faculty, major, password, is_admin, status, code)
                VALUES ('$first_name', '$last_name', '$email1', '$file_no', '$phone_number', '$faculty', '$major', '$hashedPassword', 0, 'unverified', '$verificationCode')";

        if ($conn->query($sql) === TRUE) {
            // User added successfully, send email
            $subject = "Welcome to EvLU!";
            $message = "Dear $first_name,\n\nYour User account has been created successfully by the administrator.\n\nEmail: $email1\nPassword: {$_POST['password']}\nVerification Code: $verificationCode\n\nThank you!";
            $headers = "From: jadalichahine@gmail.com";

            mail($email1, $subject, $message, $headers);

            $successMessage = "User added successfully. An email has been sent to $email1 with login details and verification code.";
        } else {
...
<?php endif; ?>
        ...
        <form method="post">
                <div class="field">
                <div class="field-body flex gap-6 justify-start">
                    <!-- First Name Input -->
                    <div class="field">
                      <label class="label">First Name</label>
                      <div class="control">
                        <input class="input" type="text" placeholder="First Name" name="first_name" value="<?php echo isset($first_name) ? $first_name : ''; ?>" required>
                      </div>
                    </div>
                    <!-- Last Name Input -->
                    <div class="field">
                      <label class="label">Last Name</label>
                        <input class="input w-full" type="text" placeholder="Last Name" name="last_name" value="<?php echo isset($last_name) ? $last_name : ''; ?>" required>
                ...
                <div class="field">
                    <label class="label">FileNo</label>
                    <div class="field-body">
                      <div class="field">
                        <div class="control">
                          <input class="input" type="text" placeholder="FileNo" name="file_no" value="<?php echo isset($file_no) ? $file_no : ''; ?>">
                        </div>
                        <p class="help">for LU students</p>
                      </div>
                    </div>
                </div>
            ...
            <div class="field">
                ...
                <div class="control">
                  <div class="select">
                    <select name="faculty" value="<?php echo isset($faculty) ? $faculty : ''; ?>">
                      <option></option>
            ...
            <div class="field">
                ...
                <div class="field-body">
                  <div class="field">
                    <div class="control">
                      <input class="input" type="text" placeholder="Major" name="major" value="<?php echo isset($major) ? $major : ''; ?>">
                    </div>
                  </div>
                </div>
                <p class="help">for LU students</p>
...
if (isset($_POST["submit_delete_user"])) {
    ...
    if (empty($user_email1)) {
        $error_message1 .= "User email is required.<br>";
    } elseif (!filter_var($user_email1, FILTER_VALIDATE_EMAIL)) {
        ...
        $check_query = "SELECT * FROM users WHERE email = '$user_email1'";
        $result = $conn->query($check_query);

        if ($result->num_rows > 0) {
            // Email found, proceed with deletion
            // SQL query to delete user by email
            $delete_query = "DELETE FROM users WHERE email = '$user_email1'";
            
            if ($conn->query($delete_query) === TRUE) {
                $success_message1 = "User with email '$user_email1' has been deleted successfully.";
            } else {
                $error_message1 = "Error deleting user: " . $conn->error;
            }
        } else {
            $error_message1 = "User with email '$user_email1' not found in the database.";
        }
                ...
                <div class="field-body">
                    <div class="field">
                        <div class="control icons-left">
                            <input class="input" type="email" placeholder="User email" name="user_email1">
                    ...
                    <div class="field grouped">
                        <div class="control">
                            <button type="submit" class="button green" name="submit_delete_user">
                                Delete User
                            </button>
                        </div>
                        <div class="control">
